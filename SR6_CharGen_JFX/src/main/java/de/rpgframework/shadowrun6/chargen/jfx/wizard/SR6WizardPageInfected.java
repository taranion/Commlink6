 ...
 *
 */
public class SR6WizardPageInfected extends WizardPage implements ControllerListener{

	private final static Logger logger = System.getLogger(SR6WizardPageInfected.class.getPackageName());
	...
	private DataItemPane<SR6Quality> contentPane;
	...
	private Comparator<SR6Quality> comparator;
	private ChoiceSelectorDialog<SetItem, SetItemValue> selectorDialog;

	//-------------------------------------------------------------------
	public SR6WizardPageInfected(Wizard wizard, GeneratorWrapper charGen) {
		super(wizard);
		this.charGen = charGen;
		setTitle(ResourceI18N.get(RES, "page.infected.title"));
		initComponents();
		initContentPane();
		initLayout();
		initInteractivity();
	...
	private void initComponents() {
		...
		comparator = new Comparator<SR6Quality>() {
			public int compare(SR6Quality meta1, SR6Quality meta2) {
...
//		selection.setAvailablePlaceholder(ResourceI18N.get(RES, "page.infected.placeholder.available"));
//		selection.setSelectedPlaceholder(ResourceI18N.get(RES, "page.infected.placeholder.selected"));
//
...
//				Shadowrun6Tools.modificationResolver(Locale.getDefault()));
	...
	private void initContentPane() {
		...
		contentPane = new DataItemPane<SR6Quality>(
				Shadowrun6Tools.requirementResolver(Locale.getDefault()),
				Shadowrun6Tools.modificationResolver(Locale.getDefault())
				);
		...
		contentPane.setImageConverter(new Callback<SR6Quality,Image>(){
			public Image call(SR6Quality value) {
				String key = value.getId();
				if (key.startsWith("ghoul_")) key="ghoul";
				if (key.startsWith("nosferatu_")) key="nosferatu";
				if (key.startsWith("vampire_")) key="vampire";
				if (key.startsWith("wendigo")) key="wendigo";
				if (key.startsWith("mutaqua")) key="mutaqua";
				String name = "images/metatypes/infected_"+key+".jpg";
		...
		contentPane.setNameConverter( meta -> {
			if (meta==null) return "-";
			String suffix = " ("+meta.getKarmaCost()+" Karma)";
...
//			SplitterJFXUtil.openDecisionDialog(r, c, null);
		...
		List<SR6Quality> items = Shadowrun6Core.getItemList(SR6Quality.class).stream()
				.filter(p -> charGen.showDataItem(p))
				.filter(p -> p.getType()==QualityType.INFECTED)
		...
		if (!items.isEmpty()) {
			charGen.getQualityController().select(items.get(0));
...
//		selection.refresh();
	...
	// -------------------------------------------------------------------
	private void userSelects(SR6Quality toSelect) {
...
//					}
//				}
				charGen.getQualityController().select(toSelect);
